    ...
    pub fn from_array_exact(array: [T; CAP]) -> Self {
        Self {
            ...
            inner: unsafe {
                array
                    .as_ptr()
                    .cast::<[std::mem::MaybeUninit<T>; CAP]>()
                    .read()
            },
        }
    }
...
mod test {
    ...
    #[test]
    fn max_vec_push_get() {
        let mut max_vec: MaxVec<usize, 5> = MaxVec::new();
        ...
        assert_eq!(max_vec.get(0), Some(&0));
        assert_eq!(max_vec.get(1), Some(&1));
        assert_eq!(max_vec.get(2), Some(&2));
        assert_eq!(max_vec.get(3), Some(&3));
    ...
    #[test]
    fn max_vec_read_from_ooc_truncate() {
        let source = [0, 1, 2, 3];
    ...
    #[test]
    fn max_vec_read_from_insufficient_data() {
        let source = [0, 1, 2, 3];
    ...
    #[test]
    fn max_vec_read_from_insufficient_into_alt_source() {
        let source1 = [0, 1, 2, 3];
        let source2 = [9, 8, 7, 6];
